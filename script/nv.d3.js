...
nv.models.discreteBar = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      y   .domain(yDomain || d3.extent(d3.merge(seriesData).map(function(d) { return 1.5*d.y }).concat(forceY))).nice();
        ...
        //.delay(function(d,i) { return i * 1200 / data[0].values.length })
          .attr('transform', function(d,i) {
            var left = x(getX(d,i)) + x.rangeBand() * .05,
                top = getY(d,i) < 0 ?
                        y(0+prev) :
                        y(0+prev) - y(getY(d,i)+prev) < 1 ?
                          y(0+prev) - 1 : //make 1 px positive bars show up above y=0
                          y(getY(d,i)+prev);
              //force position of last bar as negative value so that it hits the  0 line
              if(i==data[0].values.length-1) top = y(0+prev)-1;
              prev=prev+getY(d,i);
              d.prev= prev;
              //console.log(d, i, data[0].values[i], top);
              //nv.log([y(0+prev) - y(getY(d,i)+prev) < 1, prev].join(''));              
...
nv.models.discreteBarChart = function() {
  ...
  var showTooltip = function(e, offsetElement) {
    var left = e.pos[0] + ( offsetElement.offsetLeft || 0 ),
        top = e.pos[1] + ( offsetElement.offsetTop || 0),
        prev = e.series.values[e.pointIndex].prev,
        x = xAxis.tickFormat()(discretebar.x()(e.point, e.pointIndex)),
        y = yAxis.tickFormat()(discretebar.y()(e.point, e.pointIndex)),
        ser = e.series.values[e.pointIndex],
        content = tooltip(e.series.key, x, y, e, chart);
        //console.log(e.series.values[e.pointIndex]);
        //console.log(e.point)
        console.log(e);
        top = (ser.value>1) ? top - 150 : top - 250; 
